...
ELS2_LP2::ELS2_LP2(Instance* Ins_In, Heuristic* Heur_In, Parameters* pIn) : Model(Ins_In, Heur_In, pIn)
{
	userCutCounter = 0;
	callbackCounter = 0;
	cutTime = 0;
...
IloConstraintArray ELS2_LP2::FindUserCuts()
{
	callbackCounter++;//count the number of times this callback is entered

	NumArray4 Val_Tijtt = CreateNumArray4(env, Ins->U, Ins->P, Ins->T, Ins->T);
	...
	for (int i = 0; i < Ins->U; ++i)
		for (Product* j : Ins->CPUs[i].Products)
			for (int t = 0; t < Ins->T; ++t)
				for (int tp = t; tp < Ins->T; ++tp)
					Val_Tijtt[i][j->ID - 1][t][tp] = cplex->getValue(Tijtt[i][j->ID-1][t][tp]);
		...
		for (int j = 0; j < Ins->P; ++j)
		{
			...
			for (int q = 0; q <= l; ++q)
			{
				...
				for (int t1 = q; t1 < Ins->T; ++t1)
					for (CPU* i : Ins->Products[j].CPUs)
						sumT += Val_Tijtt[i->ID-1][j][q][t1];
...
void ELS2_LP2::Output(ofstream& ofc)
{
	Model::Output(ofc);
	ofc << "\t" << userCutCounter
		<< "\t" << Total_NConst
