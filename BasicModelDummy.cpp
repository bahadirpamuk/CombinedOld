...
BasicModelDummy::BasicModelDummy(Instance* Ins_In, Heuristic* Heur_In, Parameters* pIN) : Model(Ins_In, Heur_In, pIN)
...
#pragma endregion
	...
	LPRLX->add(Dummy <= 0); //addition of dummy variable
	cplex = new IloCplex(env);
#pragma region Export Model
	//cplex->exportModel("BasicModelDummy.lp");
#pragma endregion
};

BasicModelDummy::~BasicModelDummy()
{
	delete cplex;
	delete model;
	delete LPRLX;
...
void BasicModelDummy::Output(ofstream& ofc)
{
	Model::Output(ofc);
	if (P->mode == 0)
	{
...
IloConstraintArray BasicModelDummy::FindUserCuts()
{
